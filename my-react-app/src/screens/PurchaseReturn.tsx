...
========================================================= */
  ...
  const handleSave = async () => {
    if (!formData.purchaseNo) {
      toast.error("Please select Purchase No.");
      return;
    }
    ...
    const invalidItem = items.some((item) => Number(item.returnQty || 0) <= 0);

    if (invalidItem) {
      toast.error("Please enter Return Qty for the items.");
      return;
    }

    if (!purchaseReturnCalculation) {
      toast.error("Please calculate the purchase return before saving.");
      return;
    }
    ...
    ===================================================== */
      ...
      if (!master) {
        toast.error("Purchase master details not found.");
        return;
      }
    ...
    ===================================================== */
      ...
      const details = items
        ...
        .map((item) => ({
          ...
          prItemQty: Number(item.returnQty || 0),
          ...
          praQty: Math.max(
            0,
            Number(item.qty || 0) - Number(item.returnQty || 0),
          ),
          ...
          total: Number(item.returnQty || 0) * Number(item.rate || 0),
          ...
          taxCode: Number((item as any).taxCode || 0),

          taxName: (item as any).taxName || "",
    ...
    ===================================================== */
      ...
      if (!response?.success) {
        toast.error(response?.message || "Failed to save Purchase Return");
        return;
      }
    ...
    ===================================================== */
      ...
      const createdPRNo = Number(formData.transactionNo || 0);

      console.log("Created Purchase Return No:", createdPRNo);

      if (createdPRNo) {
        try {
          const printResponse = await getPurchaseReturnOrderPrintList(
            branchCode,
            createdPRNo,
          );

          console.log("Purchase Return Print Response:", printResponse);

          if (printResponse?.success && Array.isArray(printResponse?.data)) {
            /*
             * API response:
             *
             * data: [
            ...
            setPrintData(printResponse.data[0]);
    ...
    ===================================================== */
      ...
      toast.success(response?.message || "Purchase Return saved successfully");
            ...
            >
              ...
              <div className="grid grid-cols-2 border border-gray-800">
                ...
                <div className="p-3">
                  ...
                  <div className="mb-2">
                    <span className="font-semibold">PR Date:</span>{" "}
                    {formatPrintDate(printData.master?.prDate)}
                  </div>
                  ...
                  <div>
                    <span className="font-semibold">Purchase No:</span>{" "}
                    ...
                    {printData.details?.map((item: any, index: number) => (
                      <tr key={index}>
                        ...
                        <td className="border border-gray-800 px-2 py-2 text-right">
                          ₹ {Number(item.prItemRate || 0).toFixed(2)}
                        </td>

                        <td className="border border-gray-800 px-2 py-2 text-right">
                          {item.pReturnQty ?? item.prItemQty ?? 0}
                        </td>
              ...
              <div className="mt-6 flex justify-end">
                <div className="w-[330px]">
                  ...
                  <div className="flex justify-between border-b border-gray-300 py-2">
                    ...
                    <span>
                      ₹ {Number(printData.master?.taxAmount || 0).toFixed(2)}
                    </span>
                  </div>
